      ...
      <div className="w-full px-6 md:px-12 h-14 md:h-20 flex items-center justify-between relative">
        ...
        <div className="flex items-center">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-800">
            ...
            {useStore().categories
              ...
              .map(parent => {
                const children = useStore().categories.filter(c => c.parentId === parent.id);
                const hasChildren = children.length > 0;

                if (hasChildren) {
                  return (
                    <div key={parent.id} className="relative group cursor-pointer flex items-center gap-1 hover:text-black transition-colors">
                      <Link to={`/products?category=${encodeURIComponent(parent.slug || parent.name)}`}>
                        {parent.name}
                      </Link>
                      <ChevronDown size={14} />
                      <div className="absolute top-full left-0 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                        <div className="bg-white border border-gray-100 shadow-xl rounded-none py-2 w-48 flex flex-col">
                          {children.map(child => {
                            const grandChildren = useStore().categories.filter(c => c.parentId === child.id);
                            ...
                            return (
                              <div key={child.id} className="relative group/sub">
                                <Link
                                  ...
                                  className="px-4 py-2 hover:bg-gray-50 text-gray-600 hover:text-black text-sm flex items-center justify-between w-full"
                                >
                                  {child.name}
                                  {hasGrandChildren && <ChevronDown size={12} className="-rotate-90" />}
                                </Link>

                                {hasGrandChildren && (
                                  <div className="absolute top-0 left-full pt-0 opacity-0 invisible group-hover/sub:opacity-100 group-hover/sub:visible transition-all duration-200 z-50">
                                    <div className="bg-white border border-gray-100 shadow-xl rounded-none py-2 w-48 flex flex-col ml-0.5">
                                      {grandChildren.map(grandChild => (
                                        <Link
                                          key={grandChild.id}
                                          to={`/products?category=${encodeURIComponent(grandChild.slug || grandChild.name)}`}
                                          className="px-4 py-2 hover:bg-gray-50 text-gray-600 hover:text-black text-sm"
                                        >
                                          {grandChild.name}
                                        </Link>
                                      ))}
                ...
                return (
                  <Link
                    key={parent.id}
                    to={`/products?category=${encodeURIComponent(parent.slug || parent.name)}`}
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <Link to="/" className="flex items-center justify-center">
            <img
              src={storeInfo.logo_url || "https://ytidovmyivmhocawgbda.supabase.co/storage/v1/object/public/product-images/vnv-logo.png"}
          ...
          <nav className="p-5 flex flex-col gap-4">
            ...
            {useStore().categories
              ...
              .map(parent => {
                ...
                return (
                  <div key={parent.id} className="py-2 border-b border-gray-50">
                    {hasChildren ? (
                      <>
                        <div className="font-medium text-lg mb-2 text-gray-800 flex items-center justify-between">
                          <Link to={`/products?category=${encodeURIComponent(parent.slug || parent.name)}`} onClick={() => setIsMobileMenuOpen(false)}>{parent.name}</Link>
                          {/* Optional: Add chevron if we want expand/collapse logic, but for now kept simple */}
                        </div>
